...
//funcao para auxilio da ordenacao por selecao
//converter tudo para minusculo
//aponta para o inicio dos arrays que recebe
void minusculo(char* formatado, char* original)
{
    int i = 0;
    while(original[i] != '\0')
    {
    ...
    //fechar string com \0 (loop para antes de copiar terminador)
    formatado[i] = '\0';
} //end minusculo

//funcao swap para auxilio da ordenacao por selecao
//recebe ponteiros
void swap(Veiculo* a, Veiculo* b)
{
    //temp recebe o valor apontado por a
    Veiculo temp = *a;
    //a passa a apontar para o conteudo apontado por b
    *a = *b;
    //b aponta para temp, valor original de a
    *b = temp;
} //end swap

//ordenacao por selecao
void ordenacaoModelo(Veiculo *array, int n)
{
    //vai ate a penultima posicao (ultima ja estara ordenada)
    //depois de cada iteracao posicao i ja esta ordenada
    for(int i = 0; i < (n - 1); i++) 
    {
        //assumir que o elemento na posicao i é o menor
        int menor = i;
        //comeca do menor + 1 (elementos depois de i)
        for(int j = (i + 1); j < n; j++)
        {
            //buffer temporario, usado para deixar tudo minusculo
            char modelo1[40];
            char modelo2[40];
            //realizar conversao
            minusculo(modelo1, array[menor].modelo);
            minusculo(modelo2, array[j].modelo);
            //strcmp retorna >0 se a primeira é maior que a segunda
            if(strcmp(modelo1, modelo2) > 0)
            {
                //indice do novo menor
                menor = j;
            }
        }
        //troca o conteudo das posicoes
...
int main()
{
    ...
    ordenacaoModelo(selecionados, n);
