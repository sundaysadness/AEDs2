...
//funcao para encontrar o maior valor no array, auxilio para Radixsort
...
//ordena por um unico digito, indicado por exp
void ordenacaoAno(Veiculo* array, int n, int exp)
{
    //tamanho 10 pois estamos usando base 10
    int count[10];
    //guarda temporariamente o resultado ordenado dessa etapa
    Veiculo* output = malloc(n * sizeof(Veiculo));
    //inicializar cada posicao do array de contagem 
    for(int i = 0; i < 10; count[i] = 0, i++);
    for(int i = 0; i < n; i++) 
    {
        //isola o digito (unidade, dezena, ...) desejado
        //count[...]++ aumenta a posicao correspondente no array count
        count[(array[i].ano/exp) % 10]++;
    }
    //acumular valores, count[i] contem o numero de elemento menores ou iguais a i
    for(int i = 1; i < 10; i++) 
    {
        count[i] += count[i-1];
    }
    //ordenando de tras pra frente
    for(int i = n-1; i >= 0; i--) 
    {
        output[count[(array[i].ano/exp) % 10] - 1] = array[i];
        count[(array[i].ano/exp) % 10]--;
    }
...
void radixsort(Veiculo *array, int n) {
    //encontrar o maior para saber quantas casas decimais precisam ser analisadas
    int max = getMaior(array, n);
    //exp=1 ordema pelas unidades, exp=10 ordena pelas dezenas, exp=100 ordena pelas centenas...
    for(int exp = 1; max/exp > 0; exp *= 10) 
    {
        ordenacaoAno(array, n, exp);
    }
} //end radixsort
